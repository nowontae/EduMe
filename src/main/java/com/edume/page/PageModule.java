...
public class PageModule {
	...
	public static String boopstrapPage(String pagename,int totalCnt,int cp,int listSize, int pageSize) {
		//pageTest.jsp 에서 가져와서 사용
		...
		for(int i=(userGroup*pageSize)+1;
				i<=(userGroup*pageSize)+pageSize;i++){//1페이지 부터 총 페이지까지 
			//append : 데이터를 현재 문자열 끝에 추가한다.
			sb.append("<li class=\"page-item\"><a class=\"page-link\" href='");//속성의 시작이라 ' 붙임
			sb.append(pagename);
			sb.append("?cp=");
			sb.append(i);
			sb.append("'>");
